...
// Try multiple connection strings in sequence since replica set connectivity can be tricky
const uris = [
...
async function run() {
  try {
    const csvPath = 'C:\\Users\\ELCOT\\Downloads\\amazon scrap\\amazon_photography_accessories_1000.csv';
    ...
    console.log(`Found ${rawData.length} rows in CSV.`);
    ...
    rawData.forEach((row, i) => {
      if (!row.title || !row.price) {
        return; // Skip empty/invalid rows
      }
      ...
      if (row.link) specs.set('Amazon Link', String(row.link).trim());
      ...
      if (row.link) specsRecord['Amazon Link'] = String(row.link).trim();

      // Image - strictly use Amazon image URL (no AI generated graphics)
      let image = row.image || '';
      if (!image || (!image.startsWith('https://m.media-amazon.com') && !image.startsWith('https://images-na.ssl-images-amazon.com') && !image.startsWith('https://images.unsplash.com'))) {
        // Fallback to a real photography placeholder if empty or invalid
        image = 'https://images.unsplash.com/photo-1516035069371-29a1b244cc32?w=500'; // Real Canon camera photograph on Unsplash
      }
    ...
    // Write src/lib/accessoriesMockData.ts
...
`;
    ...
    if (connected) {
      // Clear previous Excel/CSV imports
      console.log('Clearing old imported Excel/CSV accessories from MongoDB...');
